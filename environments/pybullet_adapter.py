...
class Environment:
    ...
    async def impl(self, environment_session):
        ...
        gym_env = gym.make(
            self.gym_env_name, render_mode="rgb_array" if session_cfg.render else None
        )
        ...
        gym_observation = gym_env.reset(seed=session_cfg.seed)

        print(f"gym action space: {gym_env.action_space}")
        print(f"gym obs_space: {gym_env.observation_space}")
        print(f"gym obs: {gym_observation[0]}")

        observation = observation_space.create(
            value=gym_observation[0],
