...
MuZeroSample = namedtuple(
    "MuZeroSample",
    ["state", "action", "reward", "next_state", "done", "policy", "value"],
)
        ...
        async for sample in run_sample_producer_session.get_all_samples():
            ...
            if state is not None:
                ...
                run_sample_producer_session.produce_training_sample(
                    (
                        MuZeroSample(state, action, reward, next_state, done, policy, value),
                        total_reward,
                    )
                )
    ...
    async def single_agent_muzero_run_implementation(self, run_session):
        ...
        with mp.Manager() as manager:
            ...
            try:
                ...
                async for _step, timestamp, trial_id, _tick, (
                    sample,
                    total_reward,
                ) in sample_generator:
                    ...
                    if sample.done:
                        ...
                        xp_tracker.log_metrics(
                            timestamp,
                            total_samples,
                            trial_total_reward=total_reward,
                    ...
                    while not train_worker.results_queue.empty():
                        try:
                            (
                                info,
                                serialized_model,
                            ) = train_worker.results_queue.get_nowait()
...
def make_trial_configs(run_id, config, model_id, model_version_number):
    ...
    teacher_config = ActorParams(
        name=WEB_ACTOR_NAME,
        actor_class=TEACHER_ACTOR_CLASS,
