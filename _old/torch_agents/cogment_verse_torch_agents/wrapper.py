...
import cv2
import numpy as np
from data_pb2 import PlayerAction, ContinuousAction, Observation, NDArray
...
def cog_action_from_torch_action(action):
    ...
    if dtype in (int, np.int32, np.int64):
        if isinstance(action, np.ndarray):
            return PlayerAction(discrete_action=action.item())
        if isinstance(action, list):
            return PlayerAction(discrete_action=action[0])
        return PlayerAction(discrete_action=action)
