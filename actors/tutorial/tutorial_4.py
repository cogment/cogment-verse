...
class SimpleBCTraining:
    ...
    async def impl(self, run_session):
        ...
        for (step_idx, _trial_id, sample,) in run_session.start_and_await_trials(
            ...
            if self._cfg.training.train_only_from_demonstration and not demonstration:
                continue

            observations.append(observation)
            actions.append(action)

            if len(observations) < self._cfg.training.batch_size:
                continue

            # Sample a batch of observations/actions
            batch_indices = np.random.default_rng().integers(0, len(observations), self._cfg.training.batch_size)
